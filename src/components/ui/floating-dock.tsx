  ...
  return (
    <div className={cn('relative block md:hidden', className)}>
          ...
          <motion.div layoutId="nav" className="absolute inset-x-0 bottom-full mb-2 flex flex-col gap-2">
            {items.map((item, idx) => {
              ...
              return (
                ...
                >
                  {item.href ? <a href={item.href}>{content}</a> : <button onClick={item.onClick}>{content}</button>}
                </motion.div>
              )
            })}
          </motion.div>
      ...
      <button
        ...
        className="flex h-15 w-15 items-center justify-center rounded-full bg-neutral-800"
      >
